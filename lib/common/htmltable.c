 ...
 * through processTbl, data is stored in `tbl->u.parent` and `tbl->u.cells`, and
...
static int processTbl(graph_t * g, htmltbl_t * tbl, htmlenv_t * env)
{
    ...
    cells = tbl->u.cells = gv_calloc(cnt + 1, sizeof(htmlcell_t *));
    for (uint16_t r = 0; r < LIST_SIZE(&rows); ++r) {
	row_t *rp = LIST_GET(&rows, r);
	uint16_t c = 0;
...
static void set_cell_widths(htmltbl_t *table) {
  ...
  for (htmlcell_t **i = table->u.cells; *i != NULL; ++i) {
...
static void pos_html_tbl(htmltbl_t *tbl, boxf pos, unsigned char sides) {
    ...
    if (tbl->u.parent && tbl->u.parent->data.pencolor && !tbl->data.pencolor)
	tbl->data.pencolor = gv_strdup(tbl->u.parent->data.pencolor);
