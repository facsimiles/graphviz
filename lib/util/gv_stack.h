...
/// This implements a generic stack in C17 which is as close to the dynamically
/// expanding vectors and the like in C++ as possible.

/// WARNING: There are a few things users should be aware of:
///
/// 1. As this is C17 code, there are no object constructors or default
/// initialization of memory.  This means that if you allocate a
/// `GV_STACK(mytype) value;` then it will not be a valid value.  You must
/// explicitly initialize every `GV_STACK(mytype)` object by setting fields to
/// 0:
///
///      GV_STACK(mytype) a_stack = {0};
///
/// If you allocate a pointer, you can initialize it with a pointer to
/// zero-filled allocated memory, e.g.:
///      typedef GV_STACK(mytype) mytype_stack_t;
///      ...
///      mytype_stack_t *stack_ptr = gv_alloc(sizeof(mytype_stack_t));
///
/// 2. The preceding point illustrates that you likely want to use `typedef`
///    to create an alias for any `GV_STACK(mytype)` which you use, unless all
///    uses are contained within a single code block or function.
///    Each instance of the term `GV_STACK(mytype)` refers to a new unique
///    generic `struct`, and thus is incompatible with types declared
///    anywhere else.  Using a `typedef` makes it possible to pass stacks
///    around the program
///
/// 3. As there are also no object destructors, you should explicitly free
///    memory allocated by the `GV_STACK` when you are done using it, with
///      GV_STACK_FREE(&a_stack);
///      GV_STACK_FREE(stack_ptr)
///    When the stack is first created, it has not yet allocated memory for
///    any items, so it is safe to just drop it if it is unused.
...
///   1. The genericity of `GV_STACK` is implemented through a `union` that
///      overlaps the `base` member of a `gv_stack_t_` (a generic stack core)
///      with a typed pointer. This design involves the public macros dealing in
///      the `GV_STACK(foo)` type while the private functions deal in
///      `gv_stack_t_`s.
///
...
/// Note that things may break if any macro parameters have side-effects,
/// or refer to the stack being modified.  Note however, that:
///   • Currently, non-stack parameters (e.g., `item`, `index`) are
///     evaluated *before* any modification is done, but best to avoid
///     future risks.
///   • Parameters referring to the stack are often evaluated multiple times,
///     so pre-evaluating any expressions into a single variable is a good idea.
///
/// *** In particular, note that: internal stack references returned by
/// operations (such as `GV_STACK_BACK`) will probably be invalidated by any
/// operation modifying the stack.
